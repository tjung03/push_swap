...
**두 스택과 제한된 연산으로 정수를 오름차순으로 정렬하는 명령열을 생성하는 C 프로그램입니다.** 42 Seoul 프로젝트로, 작은 입력 전용 분기와 두 pivot을 사용하는 재귀 분할 정렬을 구현하고, 출력 전 명령을 결합·상쇄합니다.
...
현재 코드의 출력 명령을 별도 스택 시뮬레이터로 재생하여 다음 입력에서 정렬 결과를 확인했습니다.

| 입력 | 확인 |
|---|---|
| 원소 수 1~5 | 전체 순열 153개 |
| 원소 수 6·10·100·500 | 크기별 서로 다른 정수 표본 3개 |
| 오류 입력 | 중복·32비트 범위 초과·잘못된 문자 표본 |
