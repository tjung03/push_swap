...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/07/28 08:29:41 by tjung             #+#    #+#             */
/*   Updated: 2021/07/28 08:29:42 by tjung            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static int	recover_chunks_ba(t_global *g, t_sortool *t)
{
...
static int	classify_chunks_ba(t_global *g, t_sortool *t, int size)
{
	while (size--)
	{
...
static int	not_more_three_ba(t_global *g, t_sortool *t, int size)
{
...
int	sort_b_to_a(t_global *g, int size)
{
	...
	if (size < 3)
		return (not_more_three_ba(g, t, size));
	t->pivot = get_pivot(&g->loc_b, size);
	if (!(t->pivot))
		return (free_sortool(0, t));
	if (!classify_chunks_ba(g, t, size))
		return (free_sortool(0, t));
	free(t->pivot);
	if (!sort_a_to_b(g, t->pa_cnt - t->ra_cnt))
		return (free_sortool(0, t));
	if (!recover_chunks_ba(g, t))
		return (free_sortool(0, t));
